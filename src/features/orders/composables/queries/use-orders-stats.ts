...
/** Order stats, optionally scoped to a single event and/or a customer search. */
export function useOrdersStatsQuery(eventId: Ref<string | null>, search: Ref<string | null>) {
  return useQuery({
    queryKey: computed(() =>
      ORDER_QUERY_KEYS.stats(eventId.value ?? undefined, search.value ?? undefined),
    ),
    queryFn: async () => {
      const params: Record<string, unknown> = {}
      if (eventId.value) params.eventId = eventId.value
